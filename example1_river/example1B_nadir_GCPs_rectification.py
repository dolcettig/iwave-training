...
fps = 30                        # video frame rate (fps)
crs = 25832                     # EPSG CRS zone
z_0 = 442.7                     # reference water surface level
h_ref = 0.0                     # water surface deviation from reference
target_res = 0.04               # target image resolution (m/pxl)
...
dst_list = df[["Easting", "Northing", "Elevation"]].values.tolist()
...
t1 = time.time()
vid = pyorc.Video(videopath, camera_config=cam_conf, h_a=0.0, lazy=True, fps=fps)
fr = vid.get_frames()
